...
N_wfblip = 1333; % Strength Perturbation at the Off-target flux reduction site [Pa]
...
N_canal = 50e3; % Canal effective pressure corresponding to baseline flowrate of 0.005 [m^3/s]
...
tau_c =@(x,y,u) (heaviside(sed_l - x).*(Cf*N_bedrock).*abs(u) ...
                 ...
                 N_wfblip*exp(-abs(x-Xsuction)/delta)+N_infi).*abs(u) ...
...
%% Basal Drag formulation (without speed-multiplication)
% Coulomb-slip
basal_tau_c =@(x,y,u) (heaviside(sed_l - x).*(Cf*N_bedrock) ...
                 ...
                 (N_canal-N_infi)*(1-erf(0.2*abs(x-X_canal_center)/W_max))+ ...
                 (N_canal-N_infi)*(1-erf(0.2*abs(x-X_canal_sed_r)/W_max))+ ...
                 N_wfblip*exp(-abs(x-Xsuction)/delta)+N_infi) ...
...
plot(x_base,basal_tau_c_specific/1000, ...
    '-pentagram','LineWidth',3,'DisplayName','Q = 0.005 m^3/s','Color',basalstrength(5,:), ...
...
IceFluxXSuction1010 = trapz(x_surf,u_surf); % for X_suction = 1010 m

%IceFluxXSuction1020 = trapz(x_surf,u_surf); % for X_suction = 1020 m

%IceFluxXSuction1050 = trapz(x_surf,u_surf); % for X_suction = 1050 m

%IceFluxXSuction1100 = trapz(x_surf,u_surf); % for X_suction = 1100 m

%% Save the Basal Drag Profile for each case of Off-Target flux reduction

basaltauc_Canal50kPaY1010 = basal_tau_c_specific; % for X_suction = 1010 m

%basaltauc_Canal50kPaY1020 = basal_tau_c_specific; % for X_suction = 1020 m

%basaltauc_Canal50kPaY1050 = basal_tau_c_specific; % for X_suction = 1050 m

%basaltauc_Canal50kPaY1100 = basal_tau_c_specific; % for X_suction = 1100 m
